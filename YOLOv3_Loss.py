...
def Focal_Loss(pred_classes, gt_classes, alpha = 0.25, gamma = 2):
    with tf.variable_scope('Focal'):
        ...
        focal_loss = tf.reduce_sum(tf.abs(focal_loss)) / BATCH_SIZE
...
def YOLOv3_Loss(pred_tensors, gt_tensors, input_size, alpha = 1.0):
    ...
    conf_loss_op = Focal_Loss(pred_tensors[..., 4], gt_tensors[..., 4])
    class_loss_op = Focal_Loss(pred_tensors[..., 5:], gt_tensors[..., 5:])
    ...
    giou_loss_op = tf.reduce_sum(gt_tensors[..., 4] * bbox_scale * giou_loss_op) / BATCH_SIZE
